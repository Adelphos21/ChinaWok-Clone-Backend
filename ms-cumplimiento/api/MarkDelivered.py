# api/MarkDelivered.py (corregido multi-tenant)
import os
import json
import boto3
...
def lambda_handler(event, context):
    ...
    if item.get("pending_step") != "MARK_DELIVERED":
        return {
            "statusCode": 409,
            "body": json.dumps({"error": "Order not waiting for delivered"})
        }
    ...
    return {
        "statusCode": 200,
        "body": json.dumps({"message": "Order marked as delivered, workflow resumed", "order_id": order_id}),
    }
